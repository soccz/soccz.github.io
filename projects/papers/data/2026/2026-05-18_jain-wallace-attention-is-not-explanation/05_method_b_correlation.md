...
> **🧒 H1 시험 한 줄 요약**: "선생님이 '이 줄 봤어' 라고 가리킨 위치와, 그 줄을 *지웠을 때 점수 진짜 변한* 위치가 같은가?" 의 시험. 두 *순위 매김* 을 Kendall τ 로 비교. τ 가 1 에 가까우면 같음, 0 이면 무관, -1 이면 정반대. paper 결과: 평균 τ ≈ 0.32 — *약한 상관* 만 — H1 *실패*.
