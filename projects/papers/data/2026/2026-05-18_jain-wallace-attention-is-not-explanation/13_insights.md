...
> **🧒 쉬운 비유로 먼저**: 본 paper 가 마치 "마술쇼 의 트릭 폭로" 같다고 생각해보자. 7 년간 모두가 박수치며 봐온 마술 (= "attention 이 모델의 reasoning 을 보여준다" 라는 믿음) 의 *수도꼭지* 를 두 가지 명료한 시험 (H1 correlation + H2 counterfactual) 으로 뜯어내면, 사실 마술사의 손이 *전혀 다른 곳* 에 있었다는 사실이 드러나는 식. 이 챕터는 그 폭로 의 *구조* (왜 효과적이었나) 와 *영향* (이후 학계가 어떻게 변했나) 의 12 가지 분석.
