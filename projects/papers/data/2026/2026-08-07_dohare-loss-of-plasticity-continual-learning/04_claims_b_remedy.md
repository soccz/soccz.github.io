...
1. **"동반한다(accompanied by / associated with)"는 인과가 아니다.** 저자들의 언어 선택은 정확히 상관 수준이다. 세 지표가 **원인**인지 같은 상위 과정의 **부산물**인지는 주 결과에서 분리되지 않는다 — 연속 역전파가 듣는다는 건 강한 정황이지만 그 개입이 세 축을 **동시에** 건드리므로 병목이 어디인지는 미결이다.
2. **"다양성"이 세 지표로 충분히 포착된다.** 셋은 프록시이지 정의가 아니며, 유효 랭크가 높으면서도 학습이 안 되는 상태를 배제하지 못한다.
3. **"불가역적(irreversibly)"이라는 강한 단어.** Discussion 은 *"gradually and irreversibly lose their diversity"* 라고 쓰는데, 이 논문 자체가 재초기화로 회복시킨다 — **표준 경사하강 내부에서는** 불가역적이라는 좁은 의미로 읽어야 하며, 어휘 선택이 다소 과하다.
...
1. **가장 큰 긴장: L2 정규화는 무작위 성분이 아니다.** Fig. 1c 캡션은 가소성을 유지하는 알고리즘으로 *"continual backpropagation, L2 regularization and Shrink and Perturb"* 셋을 나란히 들고, Fig. 3c 캡션도 *"adding continual backpropagation or L2 regularization is necessary to perform well indefinitely"* 로 둘을 대등하게 놓는다. 그런데 **L2 는 순수 경사 기반 결정론적 항**(손실에 $\lambda\|w\|^2$ 를 더할 뿐)이다.
   → 초록의 *"requires a random, non-gradient component"* 는 **저자들 자신의 Fig. 1c/3c 와 긴장한다.** 온건하게 읽으면 "가중치 크기 억제(L2) 또는 다양성 재주입(재초기화) 중 하나가 필요하다"이지 "무작위 성분이 필수"는 아니다. 이 논문에서 내가 발견한 **가장 실질적인 내적 불일치**이며 `07_limits.md` 반박 1 로 정식화한다. (저자 측 방어를 재구성하면 "L2 는 가중치 크기 축만 고치므로 장기적으로 부족하다"이겠지만, 그런 장기 분리 실험이 주 결과에 없다.)
2. **"non-gradient" 의 의미가 두 겹이다** — ⓐ 경사 정보를 쓰지 않는다 ⓑ 결정론적이지 않다. 재초기화는 둘 다지만 **어느 쪽이 본질인지** 구분되지 않았다. 결정론적 재초기화도 통한다면 필요한 건 "무작위성"이 아니라 "경사 바깥"이다.
3. **효용 순위가 신뢰할 만하다.** 대상 선택이 잘못되면 이 처방은 오히려 해가 되며, 성숙 임계치 $m$ 이 그 리스크의 안전장치다.
...
뒤로 갈수록 야심은 커지고 근거는 얇아진다. 층위별 신뢰도 판정과 인용 원칙은 `07_limits.md` §6.5 에 정리했다.
