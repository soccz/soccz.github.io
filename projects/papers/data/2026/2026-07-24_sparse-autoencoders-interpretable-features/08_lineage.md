...
## 7.3-bis 왜 병행작 중 "이 논문"이 표준 인용이 되었나

Bricken(Anthropic)과 본 논문은 거의 동시에 같은 아이디어를 냈는데, 후속 연구가 **양쪽을 함께** 인용하되 방법론 도입부에서 본 논문을 자주 앵커로 삼는 데는 이유가 있다. (i) **접근성**: Anthropic 작업은 자사 내부 모델·인프라 색이 짙은 반면, 본 논문은 완전 공개된 Pythia + The Pile + 공개 코드로 누구나 재현 가능하다. (ii) **인과 증거**: 해석가능성(설명)만이 아니라 IOI 활성 패칭으로 **인과**까지 닫아, "SAE 특징이 상관이 아니라 원인"이라는 후속 주장들의 근거 문헌이 되었다. (iii) **정직한 경계 노출**(Claim 4): 재구성 손실·MLP 실패·$\alpha$ knee 부재를 명시해 후속 연구가 곧바로 공략 지점을 잡게 했다. 학계에서 "표준 인용"은 대개 가장 화려한 결과가 아니라 **가장 재현·확장하기 좋은** 결과에 붙는다는 점을 보여주는 사례다.
