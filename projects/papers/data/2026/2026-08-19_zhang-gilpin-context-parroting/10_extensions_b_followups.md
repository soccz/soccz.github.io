...
**무엇을 얻을 수 있는가.** ① **회로를 실제로 찾는 방법론** — 본 논문이 유비로 남긴 자리를 메우려면 이 논문의 측정 도구(prefix matching score, ablation 프로토콜)가 필요하다. ② **형성 동역학** — induction head 가 학습 중 언제 생기는지에 대한 서술은 내 Grokking 트랙과 직접 겹친다. "복사 회로의 급격한 출현"과 "grokking"이 같은 현상의 다른 이름일 가능성은 검토할 가치가 있다. ③ `_index.md` priority Tier 2 의 **미커버 항목**이라 우선 목록도 함께 소진된다.

**읽는 순서 권고**: 본 논문 → Olsson → 다시 본 논문 §5.2. 두 번째 읽을 때 유비의 한계(이산 정확 일치 vs 연속 근사 일치)가 보인다.
...
**무엇을 얻을 수 있는가.** **두 편을 합치면 즉시 실행 가능한 연구 설계가 나온다** — Wiliński et al. 의 개입 도구로 TSFM 내부의 복사 관련 성분을 찾고, 본 논문의 parroting 출력을 **행동 기준선(behavioral target)** 으로 삼아 "그 성분을 끄면 예측이 parroting 에서 얼마나 멀어지는가"를 잰다. [10_extensions_c_ideas.md](10_extensions_c_ideas.md) 아이디어 1 의 뼈대이며, 해체본이 이미 레포에 있어 착수 비용이 낮다.
...
**본 논문과 어떤 관계인가.** 본 논문이 **예측** 쪽 TSFM 에서 복사 전략을 지목했다면 이쪽은 **분류** 쪽에서 회로를 본다. 두 과제는 어텐션이 하는 일이 다르다 — 예측은 "다음에 무엇이 오는가"라 복사가 통하지만, 분류는 "이 전체가 무엇인가"라 **집계(aggregation) 회로**가 필요하다. 나란히 놓으면 **"과제가 회로를 결정하는가"** 라는 질문이 생긴다.

**무엇을 얻을 수 있는가.** ① [10_extensions_a_questions.md](10_extensions_a_questions.md) Q5(“stripe 외 나머지 motif 는 무엇인가”)의 답이 **분류 과제에 있을 가능성**을 검증할 통로. ② 프로필 기준 APF 의 concurrent work 이므로 **차별화 포인트를 정하려면 반드시 읽어야 한다**. ③ priority 소진에도 기여.

**읽는 순서 권고**: 본 논문(예측·행동) → Wiliński(예측·개입) → Kalnāre(분류·회로). 이 순서면 **"같은 도구가 과제를 건너갈 때 무엇이 유지되는가"** 축이 자연스럽게 잡힌다.

---

**세 편을 관통하는 한 문장**: Olsson 은 복사 회로의 존재를 보였고, 본 논문은 그 회로가 시계열 성능의 상당 부분을 만든다고 주장했으며, Wiliński 와 Kalnāre 는 그 주장을 열어볼 도구를 갖고 있다 — 세 편 사이의 빈칸이 곧 내 APF 트랙이 서야 할 자리다.
