...
근거는 `_prompt.md` §3 대원칙(**퀄리티 > 커버리지, 버킷·태그 균형은 선호이지 구속이 아니다**)과 규칙 3 의 예외 조항("프로파일 검토 후")이다. **짝 논문의 가치는 간격이 짧을수록 크다** — 전작 해체가 기억에 남아 있을 때 반증을 붙여야 두 편이 하나의 판단으로 합쳐진다. 대신 **다음 수요일 슬롯은 `fin-ts-dl` 로 상환**할 것을 `_index.md` 에 지시로 남긴다.
