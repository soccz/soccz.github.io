...
논문 전체의 주장이 이 다섯 줄 위에 얹혀 있다. 숨은 튜닝(시스템마다 다른 $D$, 여러 이웃의 평균, 후처리 스무딩)이 있다면 "파라미터 0개짜리 베이스라인"이라는 수사가 무너진다. 그래서 원문이 실제로 적은 것을 **그대로** 옮기고 **적지 않은 것**을 표시하는 게 이 파일의 목적이다.
