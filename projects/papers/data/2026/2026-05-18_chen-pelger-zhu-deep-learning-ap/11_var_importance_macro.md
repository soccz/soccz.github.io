...
**NBER recession 출처 (paper footnote 38)**: "NBER based Recession Indicators for the United States from the Peak through the Trough are taken from https://fred.stlouisfed.org/series/USRECM."

**Ensemble 처리 주의 (paper footnote 37)**: "Each ensemble fit returns a four dimensional vector of the state processes. However, it is not meaningful to average these vectors as the first state process in one fit does not need to correspond to the same process in another fit. It is only meaningful to report model averages of scalar output variables." → 본 논문 Fig 13 은 **9 ensemble 중 한 representative fit** 의 결과.
