...
> **배경 사다리**: ① 트랜스포머의 한 층은 은닉상태에 "어텐션 기여"와 "MLP 기여"를 **더한다**(residual stream). ② 따라서 어느 기여를 지우거나 되살리는 실험이 가능하다. ③ "간접효과"는 중간 매개 변수 하나를 통해 흐르는 효과만 따로 잰 값이다.
...
**숨은 전제 (저자가 당연시한 것).**
1. **"잡음으로 망가뜨린 상태"가 '그 사실을 모르는 모델'의 좋은 대리물이다.** $3\sigma_t$ 가우시안 잡음은 자연스러운 반사실이 아니라 **분포 밖 입력**을 만드는 조작이다. 저자도 이 불안을 감지해 Appendix B.4 Figure 13에서 다른 잡음을 비교하는데, 결과는 "효과가 측정 가능할 만큼 크지만 $3\sigma_t$보다는 작다" — 즉 **결론이 잡음 선택에 정량적으로 의존한다**.
2. **평균이 개별을 대표한다.** AIE는 1000개 문장의 평균이고, 부록 Figure 11이 보여주듯 개별 사례에서는 마지막 subject 토큰이 결정적이지 않은 경우가 흔하다.
3. **"복원 가능성 = 저장 위치".** 한 지점을 되돌려 예측이 회복된다는 건 그 지점이 정보를 **담고 있다**는 뜻일 수도, 단지 **통과시킨다**는 뜻일 수도 있다. 병목과 저장소는 다르다.

**쉬운 말 풀이.** 수도관에서 새는 곳을 찾는다고 하자. 관 전체에 흙탕물을 흘려보낸 뒤 딱 한 지점에만 깨끗한 물을 다시 주입한다. 그 한 지점만 바꿨는데 수도꼭지에서 깨끗한 물이 나온다면 거기가 결정적 길목이다. 저자들은 이 실험을 모든 (단어 위치, 층) 조합에서 반복했고 두 군데가 튀어나왔다 — 하나는 수도꼭지 바로 앞(예상됨), 다른 하나는 **"스페이스 니들"이라는 이름을 다 읽은 순간의 한복판**(뜻밖).
...
- **Appendix B.2**의 실토: 개별 MLP·Attn 활성을 **하나씩** 복원하면 효과가 무시할 수준이라 $[l^*-4, \ldots, l^*+5]$ **10개 층 구간을 통째로 복원**했다 — "결정적 정보가 여러 층에 걸쳐 누적된다"는 뜻이다.
- 대안 대비 우위: §2.2 말미 — 인과추적이 integrated gradients 같은 gradient 기반 saliency보다 정보량이 많다(**Figure 16**).

**숨은 전제.**
1. **"어텐션 vs MLP" 이분법이 의미 있다.** Eqn. 1에서 $m_i^{(l)}$의 입력에 $a_i^{(l)}$이 들어간다 — **MLP는 어텐션이 모아온 재료로 계산한다.** MLP를 끊으면 어텐션이 가져온 정보의 하류 처리도 함께 끊기므로, "MLP가 저장한다"와 "MLP가 어텐션의 산출물을 라우팅한다"를 이 실험만으로 분리하기 어렵다.
2. **10층 구간 복원이 '모듈의 효과'를 잰다.** 한 층씩은 효과가 없고 10층 묶음은 효과가 크다면 측정 단위가 "모듈"이 아니라 "10층짜리 블록"이다. 국소화의 해상도 주장이 이 지점에서 약해진다.
3. **각주 3의 아키텍처 가정** — Eqn. 1은 순차 배치 형태다. 저자들은 병렬 변형(GPT-J)에도 적용된다고 각주에서 밝히지만 이는 주장이지 실험 통제가 아니다.

**쉬운 말 풀이.** 공장에서 어느 기계가 제품을 만드는지 알아내려 한다. **컨베이어 벨트**(어텐션: 물건을 옮김)와 **가공기**(MLP: 물건을 바꿈)가 있다. 저자들은 가공기를 전부 꺼놓고 같은 실험을 다시 했더니, 앞쪽 공정을 아무리 정상으로 되돌려도 제품이 안 나왔다. 반대로 벨트를 껐을 때는 그런 붕괴가 없었다. 결론: 사실을 **만들어내는** 건 가공기 쪽이다. 다만 가공기는 벨트가 날라준 재료로 일하므로 "가공기가 원료를 창고에 보관한다"까지 단정하려면 한 걸음이 더 필요하다.

---

## Claim 1·2 종합 — §2.3의 국소화 가설

두 Claim을 합쳐 §2.3은 사실 연관이 **(i) MLP 모듈에 (ii) 특정 중간 층들에서 (iii) subject의 마지막 토큰 처리 시점에** 위치한다고 못박고, 여기에 대담한 한 줄을 덧붙인다 — Zhao et al. (2021)의 "층 순서를 바꿔도 행동 변화가 적다"를 근거로 **"we propose that this picture is complete… We conjecture that any fact could be equivalently stored in any one of the middle MLP layers."**

이 conjecture가 논문 전체에서 가장 강한 주장이자 가장 약한 고리다. 강한 이유는 이게 있어야 "층 18을 골라 편집한다"는 다음 장의 임의성이 정당화되기 때문이고, 약한 이유는 **다른 논문의 실험 결과를 근거로 자기 도메인의 완전성을 선언**하기 때문이다.
