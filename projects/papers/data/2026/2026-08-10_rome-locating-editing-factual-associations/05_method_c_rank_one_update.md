...
> **배경 사다리**: ① 최소제곱은 "정확히 맞출 수 없을 때 오차 제곱합이 가장 작아지는 답"을 찾는 방법이다. ② 라그랑주 승수법은 "제약을 지키면서 최소화"를 푸는 도구로, 제약마다 새 변수($\Lambda$)를 하나씩 붙인다. ③ rank-1 행렬은 $\Lambda u^\top$ 꼴 — 행렬 중 가장 "얇은" 변화다.
...
**③ 왜 이 형태인가.** 세 대안과 비교하면 선명하다. **단순 rank-1 투영** $\hat{W} = W + (v_* - Wk_*)k_*^\top/\|k_*\|^2$은 $C = I$인 특수 경우로, 기존 기억 분포를 무시해 빽빽한 방향을 그대로 밟는다(→ 이웃 오염). **경사하강 전체 최적화**(= FT)는 자유도가 너무 커 Table 4에서 NS 40.4로 무너진다. **하이퍼네트워크**(= KE, MEND)는 별도 학습이 필요하고 학습 분포 밖에서 깨져 KE-CF가 NS 6.9로 붕괴한다. Eqn. 2가 이기는 구조적 이유는 **"제약은 정확히, 나머지는 최소로"라는 요구를 그대로 수식으로 옮겨** 자유도가 정확히 필요한 만큼만 있기 때문이다.

**④ 조심할 점.** $C^{-1}$이 존재해야 하고(Appendix A: "assuming $C$ is nondegenerate") 조건수가 나쁘면 $\Lambda$가 폭발한다. $C$는 **위키피디아 10만 샘플** 추정치라 도메인이 다르면(예: 시계열 수치 토큰) 추정 자체가 무의미해진다. 그리고 Eqn. 2는 **한 개의 쌍**에 대한 해라, 두 사실을 연속 삽입하면 두 번째가 첫 번째를 훼손할 수 있다 — 후속작 MEMIT(arXiv:2210.07229)이 다루는 문제다.
...
**제약 추가** (Eqn. 7) $\hat{W}k_* = v_*$ → **라그랑지안** (Eqn. 8):
$$L(\hat{W}, \Lambda) = \tfrac{1}{2}\|\hat{W}K - V\|_F^2 - \Lambda^\top(\hat{W}k_* - v_*)$$

**미분해서 0** (Eqn. 10~11): $\hat{W}KK^\top = VK^\top + \Lambda k_*^\top$
...
**$\Lambda$ 풀기** (Eqn. 16~17) — $u^\top = (C^{-1}k_*)^\top$로 두고 제약에 대입하면 $Wk_* + \Lambda(u^\top k_*) = v_*$, 따라서
$$\Lambda = \frac{v_* - Wk_*}{(C^{-1}k_*)^\top k_*}$$

**여기서 읽어야 할 것.** 부록 첫 단락이 스스로 밝힌다 — "This derivation … is **a review of the classical solution of least-squares with equality constraints** as applied to our setting." 즉 **수학적 신규성은 0이다.** 이 논문의 기여는 새 정리가 아니라 **"트랜스포머 MLP를 이 고전 문제의 인스턴스로 읽어낸 매핑"** 이고, 그게 더 어려운 종류의 기여다.
...
$\Lambda(C^{-1}k_*)^\top$는 열벡터 × 행벡터이므로 계수가 정확히 1인데, 이건 **선택이 아니라 결과**다 — 제약이 하나($\hat{W}k_* = v_*$)이므로 라그랑주 승수도 하나이고 따라서 업데이트의 계수도 1이다. 사실 $n$개를 삽입하려면 제약이 $n$개라 rank-$n$이 되고 "최소 교란" 보장이 약해진다. 논문이 §3.7에서 "it only edits a single fact at a time"이라 못박고 대량 편집을 별도 논문(MEMIT)으로 미룬 데는 이런 구조적 이유가 있다.
