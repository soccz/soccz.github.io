...
> **🧒 한 줄 요약**: 다음 읽을 *3 paper* — (a) PatchTST (선행, ICLR 2023), (b) MOIRAI (후속, Salesforce 2024 TSFM), (c) Sprang 2024 (Concept Bottleneck for interpretability). 본 paper 의 *기술적 ancestor + descendant + interpretability companion*.
...
2. **MOIRAI 의 *iTransformer 변형 차이* 의 critical 부분?**
3. **Sprang 2024 의 *Concept Bottleneck* 의 iTransformer 적용 가능성?**

### 답변

1. **MOIRAI (Salesforce 2024)** — iTransformer 의 *direct TSFM 변형*. *Masked variate token* + *zero-shot* + *27B time series tokens pretraining*. iTransformer 이후 *foundation model* paradigm 의 *technical evolution* 의 정확한 다음 단계. APF / Grokking 트랙이 *TSFM 환경* 에서의 motif analysis 시 *직접 baseline*. **둘째**: PatchTST (선행, ICLR 2023) — *Channel Independence* 이론의 정밀화. **셋째**: Sprang 2024 — interpretability methodology.

2. **Masked pretraining**. iTransformer = *trained from scratch on each dataset*. MOIRAI = *masked variate token modeling* (BERT 방식) on *9 domain 27B tokens pretraining* → *zero-shot 추론*. *훈련 paradigm* 의 근본 차이 — *task-specific training → universal pretraining*. iTransformer 의 *technical foundation* + MOIRAI 의 *training paradigm 진화*.

3. ***부분 가능***. iTransformer 의 N×N attention map = *학습된 multivariate correlation*. Sprang 의 *Concept Bottleneck* = *interpretable intermediate representation* (e.g., "*냉방 demand cluster*"). iTransformer 의 attention map 의 *각 row* (variate $i$ 의 *모든 다른 variate 의 weight*) 를 *concept score* 로 재해석 → variate i 가 어떤 "*concept*" (variate group) 의 *member* 인가. → APF 의 *variate motif → concept 매핑* 의 직접 framework.
