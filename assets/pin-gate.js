/* PIN 게이트 — overlay 방식 (body class 의존 X)
 * 사용: <script src="/assets/pin-gate.js" data-title="페이지명" defer></script>
 * 모달이 fixed overlay로 떠 있어서 PIN 통과 시 모달만 제거하면 끝.
 */
(function(){
  ...
  // 이미 통과 (master token) → 아무것도 안 함
  try{
    if(sessionStorage.getItem(STORE_KEY)===PIN_HASH) return;
  }catch(e){}
